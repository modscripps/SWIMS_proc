function [SWd,ADd,EXd] = get_ps01run_data(section_num)
% Usage: [SWd,ADd,EXd] = get_ps01run_data(section_num);
% Gather SWIMS, ADCP data for a ps01 section, organized by run numbers.
%   section_num = 1 2 3 4 or 5 (1=LSTh, 2=HCxN, 3=HCxC, 4=HCxS, 5=HCTh);
% OUTPUT structures:
...
%% Edit to specify which section: (1=LSTh,2=HCxN,3=HCxC,4=HCxS,5=HCTh)
iSN = section_num;

%% Gather parameters for section from Run Index
load(fullfile(swimsindex,'SWIMS_Runs_ps01'))
...
if iSN<2, ADfrac = 0.85; end % BB ADCP to 85% of bottom
...
GP=get_gps_data(YD_BEG-.011, YD_END+.011, ...
    fullfile(swimsindex,'GPS_Log_ps01.mat'), swimsgridded);
SW=get_swims_data(YD_BEG-.01, YD_END+.01, ...
    fullfile(swimsindex,'SWIMS_Log_ps01.mat'), swimsgridded, ...
    {'starts','ends','th1','s1','sgth1','updown'});
AD=get_adcp_data(YD_BEG-.01, YD_END+.01, ...
    fullfile(swimsindex,'ADCP_Log_ps01.mat'), swimsgridded, ...
    {'bottomBT','u_wat','v_wat','ldayFP','ldayLP'});
...
if ~isempty(ix)
    AD.u_wat(ix,:) = []; AD.v_wat(ix,:) = []; AD.z_adcp(ix) = [];
end
ix = find(isnan(AD.bottomBT));
AD.u_wat(:,ix) = NaN; AD.v_wat(:,ix) = NaN; % lost bottom-tracking

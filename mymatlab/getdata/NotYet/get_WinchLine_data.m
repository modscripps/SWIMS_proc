function [Ldata] = get_WinchLine_data(beg_time, end_time, index_file, data_path, flag_ok)
% usage: 
%    [Ldata] = get_WinchLine_data(beg_time, end_time, index_file, data_path);
...
if ~iSet
   disp(['get_WinchLine_data: Index does not cover specified time range'])
   return
end
...
vars = {'yday_rate', 'eltim_rate', 'line_rate',...
...
for j=1:length(Ld2)
    ...
    for i=1:length(yd_jump_by)
        ix = find( LD.yday_rate>=yd_jump_aft(i) & ...
            LD.yday_rate<yd_jump_aft(i+1) );
        LD.yday_rate(ix) = LD.yday_rate(ix) + yd_jump_by(i);
        ix = find( LD.yday_out>=yd_jump_aft(i) & ...
            LD.yday_out<yd_jump_aft(i+1) );
        LD.yday_out(ix) = LD.yday_out(ix) + yd_jump_by(i);
    end
    LD.yday_rate = LD.yday_rate + yd_off;
    LD.yday_out = LD.yday_out + yd_off;
    % save data (adjusted) in yearday range
    index = find(LD.yday_rate >= beg_time & LD.yday_rate <= end_time);
    if ~isempty(index)
        for i=1:3
    ...
    index = find(LD.yday_out >= beg_time & LD.yday_out <= end_time);
    if ~isempty(index)
        for i=4:6
            temp = ['[ Ldata.' vars{i} ', LD.' vars{i} '(:,index)]'];
            temp2 = eval(temp);

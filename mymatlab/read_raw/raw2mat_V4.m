function raw2mat_V4(transname, matname, ByteRange ,ens_ind_rng)

%

% RAW2MAT_V4 translates a (portion of a) raw RDI file and outputs a matlab file

% 

% USAGE: raw2mat_V4(transname, matname, ByteRange ,ens_ind_rng);

% 

%  This program translates a single TRDI file. (Aug 2015)

%  Input arguments:  Transname is the TDRI raw file; matname is the

%     output file with matlab vectors and arrays;

%     ByteRange is the range of byte numbers in the input file to be

%     processed, default=[0,inf]; and ens_ind_rng is the min,max

%     range for indices of ensembles in the file (1 to ???);

%     I.E., range of indices, NOT of actual ensemble numbers.

%  [former input arguments inrule and outrule are obsolete, and removed]
...
%  V4 by DPW adds the ByteRange, and can be used for most TRDI files.
...
if nargin<3 || isempty(ByteRange)
   ...
   ByteRange = [0 inf]; % default, all in-file indices

end

if nargin<4 || isempty(ens_ind_rng)

    ens_ind_rng = [1 inf];

end % check for max ens_nos later, from data

% effective range is intersection of ByteRange and ens_ind_rng



%keyboard

% assign the following to mimic older version (V3) file handling

nfiles = 1;

in_files{1} = transname;

if length(matname)<5 || ( length(matname)>4 && ...
        ~strcmp(matname(end-3:end), '.mat') )
...
RDILastByte = -1; % to report last byte actually translated
...
% now loop through all the files [V4: only one input file and output file]
...
for file_no=1:nfiles %% For SC files, can really only do one file at a time - dpw 11/04
   ...
   while bct>0 && RD_NXT<ByteRange(2)
       ...
       [A,bct] = fread(fin, min(RD_BYTS,ByteRange(2)-RD_NXT+1), ...
            'uchar'); % read another chuck
   ...
   iEe = min(ens_ind_rng(2),length(iEpos));
      ...
      for data_type=1:ndata
         ...
         switch data_id
             ...
             case 0  % hex bytes = 00 00
                 ...
                 % Stop reading and saving most fixed leader fields after 

                 %    the first ten ensembles, they shouldn't vary anyway.
         ...
         RDILastByte = max(RDILastByte, ftell(fin));
          ...
          for i=1:length(unique(datIDok))
              ...
              switch datIDok(i)
                  ...
                  case 1536
                      ...
                      for iv=1:length(btleader)

                          eval([btleader(iv).name ' = x;']);
                  ...
                  case 8192
                      ...
                      for iv=1:length(navdata)

                          eval([navdata(iv).name ' = x;']);
              ...
              % allocate indicated arrays for 2nd pass (vectors done above)
...
if ensembles>1 % was 0, DPW 3-2001
   ...
   ensembles=0;
    ...
    ensemble_number=[]; RDILastByte = NaN;

    save(matname, 'ensemble_number', 'RDILastByte');

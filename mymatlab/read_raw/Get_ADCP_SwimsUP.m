function Vel = Get_ADCP_SwimsUP(RDIname, filetype, savetype, ByteRange);

% Vel = Get_ADCP_SwimsUP(RDIname, filetype, savetype);
...
[fid,message]=fopen(RDIname,'r');



if fseek(fid, ByteRange(1), 'bof') < 0  % start reading here

    warning(['starting byte ' num2str(ByteRange(1)) ' past end-of-file: ',RDIname])
...
if filetype < 1 % either: 0 = solo profile, or <0 = one file, multi-profs
    ...
    raw2mat_V4(RDIname, TMPfil, ByteRange);
...
Vel.LastByte = RDILastByte; % last byte read (near last ensemble's end)
...
if savetype>0
    ...
    if 0 % implement this later
        ...
        vEdn=NaN*Vdn.v1_bm; vNdn=vEdn; vZdn=vEdn; vQdn=vEdn;

        for i=1:size(vEdn,1)

            v = TXF*[Vdn.v1_bm(i,:); Vdn.v2_bm(i,:); Vdn.v3_bm(i,:); Vdn.v4_bm(i,:)];

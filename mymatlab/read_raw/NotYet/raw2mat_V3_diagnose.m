...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% local_velread %%%%%%%%%%%%%%%%%%%%%%
function local_velread(fin,data_pos,ensemble,nbins);
% read in the velocity data... 
fseek(fin,data_pos,'bof');
A=reshape(fread(fin,nbins*4,'short'),4,nbins)';
global v1 v2 v3 v4
v1(1:nbins,ensemble)=A(:,1);v2(1:nbins,ensemble)=A(:,2);
v3(1:nbins,ensemble)=A(:,3);v4(1:nbins,ensemble)=A(:,4);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% local_velread %%%%%%%%%%%%%%%%%%%%%%
function local_corread(fin,data_pos,ensemble,nbins);
% read in the velocity data... 
fseek(fin,data_pos,'bof');
A=reshape(fread(fin,nbins*4,'uchar'),4,nbins)';
global cor1 cor2 cor3 cor4
cor1(1:nbins,ensemble)=A(:,1);cor2(1:nbins,ensemble)=A(:,2);
cor3(1:nbins,ensemble)=A(:,3);cor4(1:nbins,ensemble)=A(:,4);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% local_echoread %%%%%%%%%%%%%%%%%%%%%%
function local_echoread(fin,data_pos,ensemble,nbins);
% read in the velocity data... 
fseek(fin,data_pos,'bof');
A=reshape(fread(fin,nbins*4,'uchar'),4,nbins)';
global e1 e2 e3 e4
e1(1:nbins,ensemble)=A(:,1);e2(1:nbins,ensemble)=A(:,2);
e3(1:nbins,ensemble)=A(:,3);e4(1:nbins,ensemble)=A(:,4);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% local_percread %%%%%%%%%%%%%%%%%%%%%%
function local_percread(fin,data_pos,ensemble,nbins);
% read in the velocity data... 
fseek(fin,data_pos,'bof');
A=reshape(fread(fin,nbins*4,'uchar'),4,nbins)';
global pg1 pg2 pg3 pg4
pg1(1:nbins,ensemble)=A(:,1);pg2(1:nbins,ensemble)=A(:,2);
pg3(1:nbins,ensemble)=A(:,3);pg4(1:nbins,ensemble)=A(:,4);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% local_statusread %%%%%%%%%%%%%%%%%%%%%%
function local_statusread(fin,data_pos,ensemble,nbins);
% read in the velocity data... 
fseek(fin,data_pos,'bof');
A=reshape(fread(fin,nbins*4,'uchar'),4,nbins)';
global stat1 stat2 stat3 stat4
stat1(1:nbins,ensemble)=A(:,1);stat2(1:nbins,ensemble)=A(:,2);
stat3(1:nbins,ensemble)=A(:,3);stat4(1:nbins,ensemble)=A(:,4);

% Examine_RDI_file.m  -  convert raw RDI file to matlab, compute useful quantities,
...
ccdd=pwd;
cd C:/SWIMS/data
[FilNm,PthNm] = uigetfile('AD*-*','Raw RDI file');
cd(ccdd)
...
switch FilNm(1:4)
    case 'ADDN'
        ...
        zlims = [0 110];
    case 'ADUP'
        Vel = Get_ADCP_SwimsUP(Rf, -1, 2);
        Vel.btmaxdep = []; % to indicate no BTing for logic below
        Vel.bottomBT = NaN*Vel.yday;
        zlims = [-110 0];
    otherwise
        disp('Must chose either ADDN or ADUP file, for now.')
end
...
fE = figure; subplot(2,1,2);
plot(Vel.yday,Vel.ens_no,'k.');
subplot(2,1,1)
plot(Vel.yday,[1 diff(Vel.yday)*86400],'g.')
title('\Delta T / s, ens no.  vs.  Yearday')
fA = figure;
...
for ib=1:4
    ...
    axis([Vel.yday(1) Vel.yday(end) zlims])
